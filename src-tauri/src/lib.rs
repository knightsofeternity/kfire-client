...
#[tauri::command]
fn rl_live(state: tauri::State<'_, AppState>) -> RlLive {
    RlLive {
        last_mismatch: state.db.get_setting("rl_last_mismatch").unwrap_or_default(),
...
pub fn run() {
    tauri::Builder::default()
        ...
        // running instance (which surfaces its window) and then exits, so the
        // user never ends up with several KFIRE processes and tray icons.
